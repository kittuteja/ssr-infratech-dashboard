...
const pages = { '/': '/index.html', '/login': '/login.html', '/owner-setup': '/setup.html', '/password': '/password.html', '/users': '/users.html' };
...
export default { async fetch(request, env) {
  try {
    ...
    if (!publicAssets.has(path)) {
      ...
      if (['/users', '/users.html'].includes(path) && user.role !== 'admin') return new Response('Administrator access required', { status: 403 });
    }
